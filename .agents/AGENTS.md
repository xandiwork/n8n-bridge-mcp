...
- **Ação (Requer Confirmação)**: NUNCA dispare um webhook (`n8n_executar_webhook`) ou ative/desative fluxos (`n8n_ativar_fluxo`) em produção sem antes explicar a intenção ao usuário e pedir confirmação explícita.
